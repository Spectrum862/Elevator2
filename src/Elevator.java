...
public class Elevator implements Runnable {
        ...
        while (true) {
            ...
            if (queue.isEmpty() == false) {
                if ((this.getPosition() - this.getFirst()) > 0) {
                    ...
                    while(semaphore==1)System.out.print("");
                    semaphore = 1;
                    queue.removeFirst();
                    semaphore = 0;
                    if (queue.isEmpty()) {
                ...
                if (!Building.floor[floorindex].isQDownEmpty()) {
                    this.setState("down");
                } else if (!Building.floor[floorindex].isQUpEmpty()) {
                    this.setState("up");
                }
                recieveHuman(this.state, this.position);
            }
    ...
    public synchronized void sortQueue() {
        if (state.equals("up")) {
            InsertionSort.sortQueueDES(queue);
        }
    ...
    public void displayPSG() {
        ...
        System.out.printf("\n%7s", "Floor");
        for (int i = 0; i < passengers.size(); i++) {
            System.out.printf("%6d", (passengers.get(i)).getFloor());
        }
        System.out.printf("\n%7s", "Direct");
        for (int i = 0; i < passengers.size(); i++) {
            System.out.printf("%6s", (passengers.get(i)).getDirection());
        }
        System.out.println();
    }

    public boolean isFull() {
        if (passengers.size() == max_psg) {
            return true;
        } else {
            return false;
        }
    ...
    public synchronized String displayQueue() {
        ...
        for (int i = 0; i < queue.size(); i++) {
            buff = buff + " " + (((Building.firstfloorPosi - queue.get(i)) / Building.floorheight) + 1);
        }
        semaphore = 0;
        return buff;
    }
    ...
    public void displayHuman(LinkedList<Human> human) {
        ...
        System.out.printf("\n%7s", "Floor");
        for (int i = 0; i < human.size(); i++) {
            System.out.printf("%6d", (human.get(i)).getFloor());
        }
        System.out.printf("\n%7s", "Direct");
        for (int i = 0; i < human.size(); i++) {
            System.out.printf("%6s", (human.get(i)).getDirection());
        }
        System.out.println();
    }
